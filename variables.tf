...
variable "backend_bucket" {
	description = "The GCS bucket name for Terraform remote state."
	type        = string
	default     = "iammwwhobuild-tfstate-bucket"
}

variable "backend_prefix" {
	description = "The prefix (folder path) in the GCS bucket for Terraform state files."
	type        = string
	default     = "terraform-iam-rbac"
}
